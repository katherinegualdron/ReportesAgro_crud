...
func ObtenerReportesActividad(w http.ResponseWriter, r *http.Request) {
	rows, err := config.DB.Query(`SELECT id_reporte_actividad, tipo_reporte, fecha_inicio, fecha_fin, id_usuario_solicitante, resultado, fecha_generacion FROM "ReporteActividad" ORDER BY id_reporte_actividad`)
	if err != nil {
...
func ObtenerReporteActividadPorID(w http.ResponseWriter, r *http.Request) {
	...
	row := config.DB.QueryRow(`SELECT id_reporte_actividad, tipo_reporte, fecha_inicio, fecha_fin, id_usuario_solicitante, resultado, fecha_generacion FROM "ReporteActividad" WHERE id_reporte_actividad = $1`, id)
...
func CrearReporteActividad(w http.ResponseWriter, r *http.Request) {
	...
	row := config.DB.QueryRow(`INSERT INTO "ReporteActividad" (tipo_reporte, fecha_inicio, fecha_fin, id_usuario_solicitante, resultado, fecha_generacion) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id_reporte_actividad, tipo_reporte, fecha_inicio, fecha_fin, id_usuario_solicitante, resultado, fecha_generacion`,
...
func ActualizarReporteActividad(w http.ResponseWriter, r *http.Request) {
	...
	row := config.DB.QueryRow(`UPDATE "ReporteActividad" SET tipo_reporte = $1, fecha_inicio = $2, fecha_fin = $3, id_usuario_solicitante = $4, resultado = $5, fecha_generacion = $6 WHERE id_reporte_actividad = $7 RETURNING id_reporte_actividad, tipo_reporte, fecha_inicio, fecha_fin, id_usuario_solicitante, resultado, fecha_generacion`,
...
func EliminarReporteActividad(w http.ResponseWriter, r *http.Request) {
	eliminarGenericoReportes(w, r, `"ReporteActividad"`, "id_reporte_actividad", "reporte_actividad")
}
